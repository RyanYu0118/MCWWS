...
| 沉浸式创造 | 手摆方块时用原版创造物品栏取货 | 指南首页开启；需客户端 1.0.6+；拿取扣商店价+100% 秒送费，分类页投放为卖出 |
...
| 客户端生存配套模组 | **1.2.7+** |
| 沉浸式创造客户端 | **1.0.6+**（按 E 开创造栏；数字键/中键拿一组会扣费；分类页投放为卖出；无 Axiom 也需装） |
...
**沉浸式创造（建筑向）：** 指南首页可开关。开启后按 **E** 打开与原版**相同的创造物品栏**（分类、搜索、底部仍是自己的生存背包），但游戏模式仍是生存：血条、饥饿、经验、受伤与挖掘进度不变，也不会因此获得创造飞行。关闭后 E 恢复生存背包。
...
客户端模组 `MCWWS_ImmersiveCreativeClient` 需 **1.0.6+**，用 Axiom 生存配套时请同时更新到 **1.2.7+**（1.2.6 及更早在 Editor 建造阶段会忽略沉浸式创造开关，按 E 仍开生存背包）。低于 1.0.6 的版本：数字键拿一组可能不扣款、中键可能无响应、分类页投放会把物品毁掉。低于 1.0.4 的版本上报槽位时**只带材质、不带 NBT**，会把背包里物品的附魔、Slimefun 数据、自定义名全部抹掉；服务端会识别并**自动为该玩家关闭**沉浸式创造。1.0.0 / 1.0.1 另有按 E 崩溃的问题。
...
| 沉浸式创造 | 开 | `mcwws.immersive-creative.use`；指南开关；需客户端 1.0.6+；拿取按商店价+100% 秒送费 |
...
| MCWWS_ImmersiveCreative | 1.0.6 | 沉浸式创造：生存 HUD 下 E 开创造栏，拿取按商店价+秒送费 |
| MCWWS_AxiomSurvival | 1.1.8 | 生存 Axiom 扣费、容器内容物价、实体操作计价、禁止切创造 |
