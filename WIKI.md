...
| 客户端生存配套模组 | **1.4.6+**（文件名 `MCWWS_AxiomSurvivalClient-1.4.6-needMCWWS_AxiomSurvival+1.1.9.jar`；可与服务端 1.1.10 搭配） |
...
额外计费：实体生成 **20** / 删除 **5** / 调整 **2**；生物群系画笔每格 **0.02**。生存中不可改世界时间、属性或切创造。受保护格（Slimefun / 不可破坏）写入后还原；超 **4096** 格则整包取消。持有 **`mcwws.axiom.survival.admin`**（默认 OP）可用 Axiom **真正破坏**这些方块（并清理 Slimefun 数据）以及**缺少领地权限**的格子；跳过扣费权限本身**不能**绕过领地或受保护格。超大笔刷拆包后，**仅包内**搬运可对冲；撤销按「放置/拆除对调 + 10 分钟内」配对。
...
| MCWWS_AxiomSurvival | 1.1.10 | 生存 Axiom 扣费、容器内容物价、实体操作计价、禁止切创造；管理员可破 Slimefun/领地保护格；适配 AxiomPaper 6 隧道包；需客户端 1.4.6 |
...
| MCWWS_AxiomSurvival | 1.1.10 | 见 E.1 |
