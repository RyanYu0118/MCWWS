...
- **领地范围**默认隐藏（Residence 挤出框）；需要时在地图 **图层 → 显示领地范围** 打开
...
**进服即切换：** 直接连接你要玩的那一端。锁空着时，第一次会提示「正在确认世界锁」，马上再连一次即可进入，登录过程不会再卡住。若世界正在另一端，这一端会暂停对方（对方玩家被提示改连节点）、等对方存盘上传、把最新文件放进 staging，然后**重启**。重启完成前这次登录会被挡住，踢出提示里带进度条和「已传/总数」；控制台大约每 2 秒打一行同样的进度。文件打成一个压缩包再传，不逐个请求小文件。插件 `logs` 目录不同步。重启后再进，才是刚同步的世界和位置。`worldsync force-lock` 只让人先进服，不会拉取地形。26.2 的方块在 `world/dimensions/.../region`，不在旧的 `world/region`。
...
| 客户端生存配套模组 | **1.4.8+**（文件名 `MCWWS_AxiomSurvivalClient-1.4.8-needMCWWS_AxiomSurvival+1.1.11.jar`；可与服务端 1.1.12 搭配） |
...
**拖入投影直接粘贴（1.4.8+）：** Editor 激活时，把 `.litematic` / `.schem` / `.schematic` / Axiom `.bp` 文件从**资源管理器直接拖进游戏窗口**，即可跳过「投影浏览器 → 导入 → 选中 → 粘贴」，直接进入放置预览。落点与 Ctrl+V 一致（准星命中面），移动鼠标定位后左键确认落块，扣费照常。一次拖多个文件只取第一个可识别的；Editor 未激活时拖放仍走原版行为（资源包等）。

**钢笔工具：** 节点只存在当前工具里，官方**没有**保存/导入轨迹，也**没有**多条独立路径。配套模组 **1.3.0+** 在钢笔选项里提供 **保存轨迹 / 导入轨迹**（JSON，默认目录 `config/axiom/mcwws-paths/`；**1.4.0+** 可存多层）。官方按 Enter 确认后会清空轨迹；**1.2.8+** 改为确认后仍保留节点（与 Paste Copy 相同）。官方一次只能拖一个节点，且 **Ctrl 会暂时隐藏 Gizmo**（方便点穿节点），所以没有 Ctrl+A；**1.3.0+** 可用 **Ctrl+A** 或面板「全选节点」选中全部（金色中心），松开 Ctrl 后拖当前节点的 **Y 轴**即可整体上移。**1.4.0+** 支持**路径图层**：Ctrl+C（或「复制图层」）复制当前选中/整层，Ctrl+V（或「粘贴为新图层」）新建互不串线的第二条路径；面板可切换/删除图层，预览与 Enter 落块会合并所有图层。**1.4.1+** 图层可显隐，并修复多图层时拖节点只能挪一格。**1.4.2+** 显隐为睁眼/闭眼图标，叉号删层；Delete 一次清空当前层；整组拖动只记一条历史；最小化跳过 Editor 叠加层。**1.4.3+** 图层面板置顶，曲线/形状/节点参数收进折叠子菜单；并修正眼睛椭圆长短轴。**1.4.4+** 每层曲线/形状/半径/闭环等工具参数彼此隔离。**1.4.5+** 支持 Ctrl 加选、Shift 连选多个图层；每层选块也独立隔离。**1.4.6+** 去掉图层名前不兼容字体的标记字符（避免显示成问号）。**1.4.7+** 修复最小化时原生内存暴涨（停止谎报 Axiom 激活、强制 Editor `isActive` 为假以阻断伪造宽高，并去掉整帧取消 `GameRenderer`）。**1.4.8+** 支持从资源管理器拖入投影文件直接粘贴（见上）。**1.3.3+** 修复 1.3.2 进游戏崩溃（Ctrl+A 锁移动的 mixin）。**1.3.2** 修了 1.3.1 的 Delete 注入崩溃。**1.3.1+** 按 Ctrl+A 时不会向左平移；更早版本 Delete 只删金色选中点。**1.2.9+** 也可 **Shift+点击** 加选部分节点。单次改块包超过 **50 万格**会被拒绝；「延伸至地面」在未加载区块会把空气一直填到基岩，长路径很容易超限——请关掉该项或沿路分段生成。远景模组看到的地形不是已加载区块。切换工具时请在 Axiom 配置的 Editor UI 里关闭 Clear Tool When Swapping，否则换工具也会清节点。

额外计费：实体生成 **20** / 删除 **5** / 调整 **2**；生物群系画笔每格 **0.02**。生存中不可改世界时间、属性或切创造。受保护格（Slimefun / 不可破坏）写入后还原；超 **4096** 格则整包取消。持有 **`mcwws.axiom.survival.admin`**（默认 OP）可用 Axiom **真正破坏**这些方块（并清理 Slimefun 数据）以及**缺少领地权限**的格子；跳过扣费权限本身**不能**绕过领地或受保护格。缺少领地权限时只取消本包并在聊天提示，**不会踢人**（需服务端 **1.1.12+**）。带花的花盆（`potted_*`）只按空花盆 `flower_pot` 计价，不另收花价。超大笔刷拆包后，**仅包内**搬运可对冲；撤销按「放置/拆除对调 + 10 分钟内」配对。

**容器：** 堆叠 / 粘贴箱子、潜影盒等时，**里面的物品按市场买价一并计入材料费**（拆除时内容物按卖价回收）。容器内若有 **Slimefun 物品**或带**自定义属性修饰符**的物品，整次操作会被拒绝，需先取出再编辑（管理员 `mcwws.axiom.survival.admin` 除外）。
...
### 8.1.1 管理员：Cursor MCP 建造桥

管理端工具（玩家无感）：本机 Cursor 可通过 MCP 调用 `MCWWS_BuildBridge`，在服内**直写方块**并执行 FAWE。HTTP **仅**绑定 `127.0.0.1:8765`，Bearer token 鉴权；不走网页商城 `:8002`，本期不接入生存创世神扣费。
...
配置要点：`http.token`（首次启动自动生成）、可选 `actor-player`（在线 OP，用于 FAWE 选区/按坐标粘贴）。重载：
...
**涨跌有上下限**：动态倍率被钳制在 **0.5 ~ 2.5** 之间，即相对基准价最多跌五成、最多涨一倍半，不会无限单边走。分类修正叠在钳制之后，因此各类商品的实际区间略有差异：
...
可视化编辑拆成多包时，仅**同一包内**搬运可对冲（见**建筑学 · 物质的改造**）。复制箱子等容器时，**内容物按物品买价计入材料、按卖价回收**；其中的粘液科技物品或自定义属性物品会整次拒绝。
...
购买 **`ultradepository.*` 时效权限**后，符合条件的拾取/掉落进入**本地统一仓库**；聊天提示可点「返回至背包」，一次取回**最近 1 分钟内**入库的该种物品（不必逐条点），空间不足则整批留存；返回后 **1 分钟内**该物不再自动入库。商店内对该商品按 **Ctrl+丢弃键** 可持久关闭或重新开启自动吸取。
...
- **HTTPS：** 樱花等内网穿透的**自签证书**会导致手机浏览器空白或证书警告。**根治**是为 `mcs.ryanstudio.work` 与 `mcsmap.ryanstudio.work` 配置可信证书（推荐 **Cloudflare 橙云代理** 或 **Let's Encrypt**），穿透只走 HTTP 到本机即可。临时方案：Android APK ≥ **1.1.2** 已对两域名放行自签；地图 Tab 依赖 `mcsmap` 子域，旧版 App 仅信任主站会在地图页误报「无法连接」
...
- 「跳过扣费」**不能**绕过领地；Axiom 管理员权限 `mcwws.axiom.survival.admin`（默认 OP）可改领地内与受保护格；其余覆盖仍走 ResAdmin / OP
...
| 跳过可视化编辑扣费 | **关** | 不能绕过领地 / Slimefun 保护 |
| 可视化编辑管理（破保护格） | **OP** | `mcwws.axiom.survival.admin`：可破坏 Slimefun/不可破坏方块与领地保护格 |
| 跳过指令批量扣费 | **关** | 不能绕过领地 |
...
| 两端世界互斥同步 | `tools/mcwws-world-sync/` → `MCWWS_WorldSync-1.2.6.jar`；数据 `plugins/MCWWS_WorldSync/` |
...
| MCWWS_WorldSync | 1.2.6 | 接管打成一个压缩包传输；日志目录不再逐个文件拉取 |
...
| MCWWS_AxiomSurvival | 1.1.12 | 见 E.1 |
