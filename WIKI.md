...
| 客户端生存配套模组 | **1.3.3+**（文件名 `MCWWS_AxiomSurvivalClient-1.3.3-needMCWWS_AxiomSurvival+1.1.9.jar`） |
...
**钢笔工具：** 节点只存在当前工具里，官方**没有**保存/导入轨迹。配套模组 **1.3.0+** 在钢笔选项里提供 **保存轨迹 / 导入轨迹**（JSON，默认目录 `config/axiom/mcwws-paths/`）。官方按 Enter 确认后会清空轨迹；**1.2.8+** 改为确认后仍保留节点（与 Paste Copy 相同）。官方一次只能拖一个节点，且 **Ctrl 会暂时隐藏 Gizmo**（方便点穿节点），所以没有 Ctrl+A；**1.3.0+** 可用 **Ctrl+A** 或面板「全选节点」选中全部（金色中心），松开 Ctrl 后拖当前节点的 **Y 轴**即可整体上移。**1.3.3+** 修复 1.3.2 进游戏崩溃（Ctrl+A 锁移动的 mixin）。**1.3.2** 修了 1.3.1 的 Delete 注入崩溃。**1.3.1+** 按 Ctrl+A 时不会向左平移，Delete 会删掉所有金色选中点（官方只删当前那一个）。**1.2.9+** 也可 **Shift+点击** 加选部分节点。单次改块包超过 **50 万格**会被拒绝；「延伸至地面」在未加载区块会把空气一直填到基岩，长路径很容易超限——请关掉该项或沿路分段生成。远景模组看到的地形不是已加载区块。切换工具时请在 Axiom 配置的 Editor UI 里关闭 Clear Tool When Swapping，否则换工具也会清节点。
...
| MCWWS_AxiomSurvival | 1.1.9 | 生存 Axiom 扣费、容器内容物价、实体操作计价、禁止切创造；适配 AxiomPaper 6 隧道包；需客户端 1.3.3 |
| MCWWS_AxiomSurvivalClient | 1.3.3 | Axiom 生存客户端，装进游戏 `mods/`；jar 含 `needMCWWS_AxiomSurvival+1.1.9` |
