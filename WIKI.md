...
**外卖柜（Slimefun / RSC）：** 增强型合成台用铁锭与箱子合成「外卖柜」，放置后在聊天依次输入**大编号**、**小编号**（可自定义，最长 24 字）。大编号即网页下单的收货地址；同址可放多台柜，小编号在该址内必须唯一。GUI 两侧木牌可改号；纸张按钮用于输入取件码。顶栏信息木桶、木牌、取件按钮与底栏装饰格**不能取出**。漏斗与货运节点**不能**从柜内抽放，须打开界面存取。拆除时内容物掉落并注销编号。
