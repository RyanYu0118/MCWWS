...
| 粘液科技指南首次进服赠送；配方书与自定义合成 GUI 全员可浏览 | 每日签到 `/signin gui`；基岩版经 Geyser 同端口进服 |
...
Slimefun **手持类物品**（便携式工作台/垃圾桶/背包/GPS 工具等，以及所有 `PORTABLE_*` 物品）只能手持右键使用，**不能放置**；放下再拆会失去史莱姆功能，变成普通方块或头颅。  
**机器类**（货运管理器、太阳能板、电力冶炼炉等）可正常放置。Skript `portable_crafter_place.sk` 在服务端强制拦截违规放置。
...
| MCWWS-rsc（RSC 附属） | — | 外卖柜、服内定制 Slimefun 物品 |
...
| CustomCrafting | 4.19.0 | 自定义配方浏览器 |
