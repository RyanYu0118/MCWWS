...
网页下单必须填写已存在的大编号，并会**随机生成 6 位取件码**（结账弹窗与待领取列表可见）。物品按小编号顺序自动塞进该址各柜。柜内网页包裹带订单标记：**仅下单账号**在对应收货地址的柜子输入正确取件码后，**10 分钟内**可取走该码对应包裹；他人或未解锁时无法取出。柜满则订单失败并重试。未编号的柜不接收网页订单。旧的无地址订单仍发到随身行囊。手工放入的无标记物品仍可从界面自由存取。
