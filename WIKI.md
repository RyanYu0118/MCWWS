...
| 客户端生存配套模组 | **1.4.6+**（文件名 `MCWWS_AxiomSurvivalClient-1.4.6-needMCWWS_AxiomSurvival+1.1.9.jar`） |
...
**容器：** 堆叠 / 粘贴箱子、潜影盒等时，**里面的物品按市场买价一并计入材料费**（拆除时内容物按卖价回收）。容器内若有 **Slimefun 物品**或带**自定义属性修饰符**的物品，整次操作会被拒绝，需先取出再编辑。
...
| MCWWS_AxiomSurvival | 1.1.9 | 生存 Axiom 扣费、容器内容物价、实体操作计价、禁止切创造；适配 AxiomPaper 6 隧道包；需客户端 1.4.6 |
