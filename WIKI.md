...
| 沉浸式创造 | 手摆方块时用原版创造物品栏取货 | 指南首页开启（重进仍保留）；需客户端 1.0.7+；拿取扣商店价+100% 秒送费，分类页投放为卖出 |
...
| 客户端生存配套模组 | **1.2.7+** |
| 沉浸式创造客户端 | **1.0.7+**（按 E 开创造栏；开关重进保留；数字键/中键拿一组会扣费；分类页投放为卖出；无 Axiom 也需装） |
...
客户端模组 `MCWWS_ImmersiveCreativeClient` 需 **1.0.7+**（1.0.7 起进服会主动向服务端核对开关，避免重进后仍打开生存背包），用 Axiom 生存配套时请同时更新到 **1.2.7+**（1.2.6 及更早在 Editor 建造阶段会忽略沉浸式创造开关，按 E 仍开生存背包）。低于 1.0.6 的版本：数字键拿一组可能不扣款、中键可能无响应、分类页投放会把物品毁掉。低于 1.0.4 的版本上报槽位时**只带材质、不带 NBT**，会把背包里物品的附魔、Slimefun 数据、自定义名全部抹掉；服务端会识别并**自动为该玩家关闭**沉浸式创造。1.0.0 / 1.0.1 另有按 E 崩溃的问题。
...
| 沉浸式创造 | 开 | `mcwws.immersive-creative.use`；指南开关（重进保留）；需客户端 1.0.7+；拿取按商店价+100% 秒送费 |
...
| MCWWS_ImmersiveCreative | 1.0.7 | 沉浸式创造：生存 HUD 下 E 开创造栏，开关重进保留，拿取按商店价+秒送费 |
