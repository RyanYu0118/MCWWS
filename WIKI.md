...
| 沉浸式创造 | 手摆方块时用原版创造物品栏取货 | 指南首页开启；需 `MCWWS_ImmersiveCreativeClient`；拿取扣商店价，血条仍为生存 |
...
| 客户端生存配套模组 | **1.2.6+** |
| 沉浸式创造客户端 | **1.0.4+**（按 E 开创造栏；无 Axiom 也需装；低于 1.0.4 会丢物品 NBT，服务端拒收） |

流程摘要：开菜单时本地旁观；建造时 `E` 默认仍是生存背包、挖掘按生存进度；若在指南开启了**沉浸式创造**，则 `E` 为原版创造物品栏（血条/经验仍为生存，拿取按商店价）。退出后位置与模式恢复，**编辑界面调的飞行速度会保留**（需客户端 1.2.6+）。进入 Editor 不会自动开飞。Editor 打开期间沉浸式创造不会抢背包（仍用生存背包，以免关掉 Axiom 工具菜单）。
...
**沉浸式创造（建筑向）：** 指南首页可开关。开启后按 **E** 打开与原版**相同的创造物品栏**（分类、搜索、底部仍是自己的生存背包），但游戏模式仍是生存：血条、饥饿、经验、受伤与挖掘进度不变，也不会因此获得创造飞行。从创造栏**取出**的物品按商店买价扣费；商店不出售或黑名单物品无法取出。关闭后 E 恢复生存背包。

在创造栏里**销毁**物品不会让它凭空消失，而是记成一笔**同材质的额度**：下次再从创造栏取同样的东西时先抵扣额度，扣完了才开始收钱。单纯把物品从一个槽位挪到另一个槽位同理，不会被当成重新购买。额度只在本次在线期间有效，**退出游戏或关闭沉浸式创造即清空**。

客户端模组 `MCWWS_ImmersiveCreativeClient` 需 **1.0.4+**，用 Axiom 生存配套时请同时更新到 **1.2.6+**。低于 1.0.4 的版本上报槽位时**只带材质、不带 NBT**，会把背包里物品的附魔、Slimefun 数据、自定义名全部抹掉；服务端 1.0.4 起会识别并**自动为该玩家关闭**沉浸式创造，提示更新模组。1.0.0 / 1.0.1 另有按 E 崩溃的问题。
...
| 沉浸式创造 | 开 | `mcwws.immersive-creative.use`；指南开关；需客户端模组；拿取按商店价 |
...
首页可开关**沉浸式创造**（按 E 打开创造物品栏、拿取扣商店价，血条与经验仍为生存）。  
...
| MCWWS_ImmersiveCreative | 1.0.4 | 沉浸式创造：生存 HUD 下 E 开创造栏，拿取按商店价 |
