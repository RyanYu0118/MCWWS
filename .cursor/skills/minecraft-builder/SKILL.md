...
**楼梯 `facing` 铁律**：所有 `*_stairs`（椅子、屋檐、倒角、踏步）的标签 `facing` **与肉眼朝向相反**——先定视觉方向，再写入反方向；放完用 `get_block` 核对，禁止凭标签字面理解。

**路改绿带跟**：改马路宽度后，两侧树叶+菌光体必须贴新路缘重铺。

**绿带不挡人行道 / 墙壁优先**：绿带不得铺在人行道上（本带 `x <= -721`）；路口 T 口留空。撞墙则终止，禁止挖墙脚穿墙。左右圆角须镜像对称。

**倒角防呆**：圆心在草地；角块内 `dist≥R` 铺路、`dist<R` 留草（路缘相对草为凸）；写块前报三区+圆心+半径；写块后 `get_block` 字符图验收。详见 `build-interior-quality.mdc`「倒角防呆」。

**碎石混铺**：优先 FAWE `//set stone,stone,andesite,cobblestone`（2:1:1，自带高随机）；不要自写铺路随机算法。

**广场接人行道**：向外圆弧连接，禁止往广场内侧切缺角。
