...
extends HierarchicalAddonModule {
    ...
    protected void initSelf(SkriptAddon addon) {
        Classes.registerClass(new EnumClassInfo<InputKey>(InputKey.class, "inputkey", "input keys").user("input ?keys?").name("Input Key").description("Represents a movement input key that is pressed by a player.").since("2.10").requiredPlugins("Minecraft 1.21.3+"));
    }

    @Override
    protected void loadSelf(SkriptAddon addon) {
        this.register(addon, CondIsPressingKey::register, EvtPlayerInput::register, ExprCurrentInputKeys::register);
    }
