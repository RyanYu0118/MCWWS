...
@ApiStatus.Internal
public final class AudienceClassInfo
extends ClassInfo<Audience> {
    public AudienceClassInfo() {
        super(Audience.class, "audience");
        this.user("audiences?").name("Audience").description("An audience is a receiver of media, such as individual players, the console, or groups of players (such as those on a team or in a world).").examples("send \"Hello world!\" to the player", "send action bar \"ALERT! A horde of zombies has overrun the central village.\" to the world").since("2.15").defaultExpression(new EventValueExpression<CommandSender>(CommandSender.class)).parser(new AudienceParser());
    }

    private static final class AudienceParser
    extends Parser<Audience> {
