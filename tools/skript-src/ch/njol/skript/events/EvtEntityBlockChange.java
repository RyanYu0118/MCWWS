...
extends SkriptEvent {
    ...
    public boolean init(Literal<?>[] args, int matchedPattern, SkriptParser.ParseResult parseResult) {
        ...
        if (this.event == ChangeEvent.GENERIC) {
            this.datas = args[0];
        }
        return true;
    }
    ...
    public boolean check(Event event) {
        ...
        if (this.datas != null && !this.datas.check(event, data -> data.isInstance(((EntityChangeBlockEvent)event).getEntity()))) {
            return false;
        }
    ...
    private static enum ChangeEvent {
        ...
        private ChangeEvent(String pattern) {
            this(pattern, null);
        }

        private ChangeEvent(String pattern, Predicate<EntityChangeBlockEvent> checker) {
