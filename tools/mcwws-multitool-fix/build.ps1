...
$MultitoolPlus = Get-ChildItem (Join-Path $Root "plugins") -Filter "MultitoolPlus*.jar" |
    Where-Object { $_.Name -notlike "*.new" } |
    Sort-Object Name -Descending |
    Select-Object -First 1 -ExpandProperty FullName
if (-not $MultitoolPlus) { throw "Missing MultitoolPlus jar in plugins/" }
...
$CpParts = @($PaperApi, $MultitoolPlus) + $Libs | Where-Object { $_ -and (Test-Path $_) }
