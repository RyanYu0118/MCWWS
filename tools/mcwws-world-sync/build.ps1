...
$McwwsJar = Get-McwwsPluginJarPaths -RepoRoot $Root -PluginName "MCWWS_WorldSync" -ResourcesDir $Res
...
$CpParts = @($PaperApi) + $Libs | Where-Object { $_ -and (Test-Path $_) }
...
& $JavacExe -encoding UTF-8 --add-modules jdk.httpserver -cp $Cp -d $Out $JavaFiles
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Copy-Item -Recurse -Force (Join-Path $Res "*") $Out
Publish-McwwsPluginJar -JarExe $JarExe -ClassesDir $Out -JarPaths $McwwsJar -PluginName "MCWWS_WorldSync"
