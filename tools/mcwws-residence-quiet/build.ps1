...
$McwwsJar = Get-McwwsPluginJarPaths -RepoRoot $Root -PluginName "MCWWS_ResidenceQuiet" -ResourcesDir $Res
...
$Residence = Get-ChildItem (Join-Path $Root "plugins") -Filter "Residence*.jar" |
    Where-Object { $_.Name -notlike "*.new" } |
    Sort-Object Name -Descending |
    Select-Object -First 1 -ExpandProperty FullName
...
$CpParts = @($PaperApi, $ProtocolLib, $Residence) + $Libs | Where-Object { $_ -and (Test-Path $_) }
...
if (-not $Residence) { throw "Missing Residence jar in plugins/" }
...
Publish-McwwsPluginJar -JarExe $JarExe -ClassesDir $Out -JarPaths $McwwsJar -PluginName "MCWWS_ResidenceQuiet"
