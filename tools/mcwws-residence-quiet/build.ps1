...
$JarOut = Join-Path $Root "plugins/MCWWS_ResidenceQuiet.jar"
$JarOutNew = Join-Path $Root "plugins/MCWWS_ResidenceQuiet.jar.new"
...
if (Test-Path $JarOut) {
    try {
        ...
        Write-Host "Built $JarOutNew (原 jar 被占用，停服后替换 plugins/MCWWS_ResidenceQuiet.jar)"
    }
} else {
    Move-Item $JarOutNew $JarOut
    Write-Host "Built $JarOut"
