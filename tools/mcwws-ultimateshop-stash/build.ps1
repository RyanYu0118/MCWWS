...
$McwwsJar = Get-McwwsPluginJarPaths -RepoRoot $Root -PluginName "MCWWS_UltimateShopStash" -ResourcesDir $Res
...
$UltimateShop = Get-ChildItem (Join-Path $Root "plugins") -Filter "UltimateShop*.jar" |
    Where-Object { $_.Name -notlike "*.new" } |
    Sort-Object Name -Descending |
    Select-Object -First 1 -ExpandProperty FullName
$PlaceholderApi = Get-ChildItem (Join-Path $Root "plugins") -Filter "PlaceholderAPI*.jar" |
    Sort-Object Name -Descending |
    Select-Object -First 1 -ExpandProperty FullName
...
$Libs = @(
    ...
    (Find-Newest "libraries/net/kyori/adventure-text-minimessage" "adventure-text-minimessage-*.jar"),
...
$CpParts = @($PaperApi, $UltimateShop, $PlaceholderApi, $BetterBags, $BungeeChat) + $Libs | Where-Object { $_ -and (Test-Path $_) }
...
Publish-McwwsPluginJar -JarExe $JarExe -ClassesDir $Out -JarPaths $McwwsJar -PluginName "MCWWS_UltimateShopStash"
