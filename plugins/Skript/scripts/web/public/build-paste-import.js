...
(function () {
    ...
    function renderQuote(quote, ctx) {
        ...
        const effectiveReplace = syncSnapshot.effectivePasteReplaceBehavior || syncSnapshot.pasteReplaceBehavior;
        const pasteReplaceLabel = effectiveReplace
            ? (sync?.labelReplaceBehavior?.(effectiveReplace) || effectiveReplace)
            : '';
        const pasteReplaceNote = syncSnapshot.pasteReplaceManual ? '（手动指定）' : '';
        const clientBlock = placement ? `
            <div class="build-paste-client-box">
                ...
                <div class="build-paste-client-grid">
                    ...
                    ${pasteReplaceLabel ? `<span>粘贴替换 ${ctx.escapeHtml(pasteReplaceLabel)}${pasteReplaceNote}</span>` : ''}
                </div>
    ...
    async function checkoutQuote() {
        ...
        try {
            ...
            const replaceBehavior = getSyncApi()?.getEffectivePasteReplaceBehavior?.();
            if (replaceBehavior) {
                payload.pasteReplaceBehavior = replaceBehavior;
            }
